...
export default {
	...
	mounted() {
		let defaultOption = {
			...
			colors: ['rgb(194,197,228)', '#2fc25b', '#facc14', '#f04864', '#8543e0', '#90ed7d'], //图表配色方案，不传则使用系统默认配置
			rotate: false, //是否横屏展示
			...
			lineCharts[this.canvasId].showToolTip(e, {
				...
				format: function(item, category) {
					...
					if (typeof item.data === 'object') {
						...
						// else if(type=='time'){
						// 	newName=name ||':';
						// 	return  `${category} ${item.name}:${(item.data.value /60).toFixed(0)}${newName}${item.data.value%60}`
						// }
						else {
						...
						// else if(type=='time'){
						// 	newName=name ||':';
						// 	return  `${category} ${item.name}:${(item.data /60).toFixed(0)}${newName}${item.data%60}`
						// }
						else {
