	...
	<view class="normal_clock">
		...
		<view class="normal_clock_content">
			...
			<view class="save_setting">
				<button class="save_button" @click="sendClock" :style="{'background-color':color}">
					<text class="save">发送设置请求</text>
				</button>
	...
	export default {
		...
		methods: {
			...
			sendClock: function() {
				...
				uni.request({
					url: 'http://106.54.76.21:8080/clocks/sendRequest',
					data: {
						gameType: type,
						gameId: that.gameid,
						time: time,
		...
		onLoad(options){
			this.receiveid=options.receiver;
